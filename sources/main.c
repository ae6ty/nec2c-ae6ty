...
//ae6ty 12-7-21 added YY card
//ad6ty 3-4-22 added MP card
void printYParams();
...
// ae6ty 12-7-21 added 'YY' card
// add '-q' flag
// 12-12-21 added 'QQ' card
// 3-8-21 added 'MP' card
...
#define NUM_CMNDS  23
  ...
  // ae6ty 12-7-21 remove check for no arguments
  /*
  if( argc == 1 )
  {
	usage();
  ...
  */
  ...
  while( (option = getopt(argc, argv, "i:o:hvq") ) != -1 )
  {
	switch( option )
	{
	  // ae6ty 12-7-21 added -q switch
	case 'q':
	  Qflag = 1;
	  break;
  ...
  if( strlen(infile) != 0 && strlen( otfile ) == 0 )
  {
  ...
  while( TRUE )
  {
		...
		(strcmp(ain, "CE") == 0) )
	{
	  ...
	  fprintf( output_fp,
		  "                              %s\n",
		  &line_buf[2] );
	  processCMLine(line_buf+2); // ae6ty 3-7-22
	...
	mreq *= sizeof(double);
	mem_realloc( (void *)&crnt.air, mreq , 39);
	mem_realloc( (void *)&crnt.aii, mreq , 40);
	mem_realloc( (void *)&crnt.bir, mreq , 41);
	mem_realloc( (void *)&crnt.bii, mreq , 42);
	mem_realloc( (void *)&crnt.cir, mreq , 43);
	mem_realloc( (void *)&crnt.cii, mreq , 44);
	mem_realloc( (void *)&xtemp,  mreq , 45);
	mem_realloc( (void *)&ytemp,  mreq , 46);
	mem_realloc( (void *)&ztemp,  mreq , 47);
	mem_realloc( (void *)&sitemp, mreq , 48);
	mem_realloc( (void *)&bitemp, mreq , 49);

	mreq = (size_t)data.np2m;
	mreq *= sizeof(int);
	mem_realloc( (void *)&save.ip, mreq , 260);

	mreq = (size_t)data.np3m;
	mreq *= sizeof( complex double);
	mem_realloc( (void *)&crnt.cur, mreq , 261);
	...
	while( ! next_job )
	{
	  ...
	  switch( ain_num )
	  {
		...
		case 1: /* "ld" card, loading parameters */
		  {
			...
			mreq *= sizeof(int);
			mem_realloc( (void *)&ldtyp,  mreq , 262);
			mem_realloc( (void *)&ldtag,  mreq , 263);
			mem_realloc( (void *)&ldtagf, mreq , 264);
			mem_realloc( (void *)&ldtagt, mreq , 265);
			mreq = (size_t)zload.nload;
			mreq *= sizeof(double);
			mem_realloc( (void *)&zlr, mreq , 266);
			mem_realloc( (void *)&zli, mreq , 267);
			mem_realloc( (void *)&zlc, mreq , 268);
		...
		case 3: /* "ex" card, excitation parameters */
		  ...
		  if( (itmp1 == 0) || (itmp1 == 5) )
		  {
			...
			if( fpat.ixtyp == 5)
			{
			  ...
			  mreq *= sizeof(int);
			  mem_realloc( (void *)&vsorc.ivqd, mreq , 270);
			  mem_realloc( (void *)&vsorc.iqds, mreq , 271);
			  mreq = (size_t)vsorc.nvqd;
			  mreq *= sizeof(complex double);
			  mem_realloc( (void *)&vsorc.vqd, mreq , 272);
			  mem_realloc( (void *)&vsorc.vqds, mreq , 273);
			...
			mreq *= sizeof(int);
			mem_realloc( (void *)&vsorc.isant, mreq , 274);
			mreq = (size_t)vsorc.nsant;
			mreq *= sizeof(complex double);
			mem_realloc( (void *)&vsorc.vsant, mreq , 275);
		...
		case 4: case 5: /* "nt" & "tl" cards, network parameters */
		  {
			...
			mreq *= sizeof(int);
			mem_realloc( (void *)&netcx.ntyp, mreq , 280);
			mem_realloc( (void *)&netcx.iseg1, mreq , 281);
			mem_realloc( (void *)&netcx.iseg2, mreq , 282);
			mreq = (size_t)netcx.nonet;
			mreq *= sizeof(double);
			mem_realloc( (void *)&netcx.x11r, mreq , 283);
			mem_realloc( (void *)&netcx.x11i, mreq , 284);
			mem_realloc( (void *)&netcx.x12r, mreq , 285);
			mem_realloc( (void *)&netcx.x12i, mreq , 286);
			mem_realloc( (void *)&netcx.x22r, mreq , 287);
			mem_realloc( (void *)&netcx.x22i, mreq , 288);
		...
		case 16: /* "cp" card, maximum coupling between antennas */
		  ...
		  mreq *= sizeof(int);
		  mem_realloc( (void *)&yparm.nctag, mreq , 290);
		  mem_realloc( (void *)&yparm.ncseg, mreq , 291);
		  ...
		  mreq = (size_t)yparm.ncoup;
		  mreq *= sizeof(int);
		  mem_realloc( (void *)&yparm.nctag, mreq , 292);
		  mem_realloc( (void *)&yparm.ncseg, mreq , 293);
		...
		case 19: /* "wg" card, not supported */
		  ...
		  break;
	  case 21: /* QQ Card */
	    fprintf(stderr,"itmp %d %d %d %d\n",itmp1,itmp2,itmp3,itmp4);
	    Qflag = itmp1;
	    fprintf(stderr,"Qflag %d\n",Qflag);
	    continue;
	  case 22: /* MP Card  procs blocksize */
	    continue;
	  case 20: /* 'YY' card */
	    //ae6ty 12-10-21  expanded YY card format.
	    Yflag = 1;
	    continue;
	  default:
	    if( ain_num != 18 )
	  ...
	  /* Allocate to normalization buffer */
	  {
		...
		{
		  if( mreq1 > mreq2 )
		    mem_realloc( (void *)&fnorm, mreq1 , 294);
		  else
		    mem_realloc( (void *)&fnorm, mreq2 , 295);
		}
	  }
...
/*  Null_Pointers()
 *
 *  Nulls pointers used in mem_reall, oc
 */
  void
Null_Pointers( void )
{
...
/*-----------------------------------------------------------------------*/
// ae6ty 12-7-21 added print y parameters.
void printYParam(int tag, int seg) {
  int segnum = 0;
  for (int i=0;i<data.n;++i) {
      if (data.itag[i] == tag) {
	++segnum;
	if (segnum == seg) {
	  Complex curi = crnt.cur[i]* data.wlam;
	  fprintf(stdout,"%11.4E %11.4E",creal(curi),cimag(curi));
	  return;
	}
      }
  }
...
void printYParams() {
  ...
  fprintf(stdout,"\n    -YY ");
  int prefix = 0;
  for (int i=0;i<numYYNums;i+=2) {
    if (prefix)
      fprintf(stdout," ");
    printYParam(YYNums[i],YYNums[i+1]);
    prefix = 1;
  }
}
