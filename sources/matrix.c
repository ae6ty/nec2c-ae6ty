...
void factr( int n, complex double *a, int *ip, int ndim)
{
  ...
  //  printf("\nfactr %d %d\n",MPNumProcessors,MPBlockSize);

  // ae6ty 2-17-22  added matrixMode.
  if (MPNumProcessors != 0) {
    for (int r=0;r<n;++r)
      ip[r] = r; 
    if (MPNumProcessors == 1) {
      //      printf("\nMP: single processor\n");
      lu_factor(a,ndim,n,ip);
    }
    else {
      //      printf("\nMP: multiProcessor %d %d\n",MPNumProcessors,MPBlockSize);
      lu_multiFactor(a,ndim,n,ip,MPBlockSize,MPNumProcessors);
    }
    return;
  }
  /* Un-transpose the matrix for Gauss elimination ae6ty 4-16-22 really is 'just transpose'*/
  for( i = 1; i < n; i++ )
	for( j = 0; j < i; j++ )
	{
	  arj = a[i+j*ndim];
  ...
  //  printMatrix("before factr",n,a,ip,ndim);
  ...
  //  copyForLU1 = copyRowMajorMatrixSetP(a,pForLU1,ndim,n);
  ...
  //  printMatrix("after factr",n,a,ip,ndim);
