...
void couple( complex double *cur, double wlam )
{
  ...
  mem_realloc( (void *)&yparm.y11a, mreq  ,1);
  ...
  for( i = 0; i < yparm.ncoup; i++ )
  {
	...
	mreq = (size_t)l1;
	mreq *= sizeof( complex double);
	mem_realloc( (void *)&yparm.y12a, mreq ,2);
	...
	double *zlr, double *zli, double *zlc )
{
  ...
  mreq *= sizeof(complex double);
  mem_realloc( (void *)&zload.zarray, mreq ,3);
  for( i = 0; i < data.n; i++ )
...
void trio( int j )
{
  ...
  if( (jcox == 0) || (jcox > PCHCON) )
  {
	...
	if( jcox == 0 || (jcox > PCHCON) )
	{
	  ...
	  if( segj.jsno >= segj.maxcon )
	  {
		...
		mreq *= sizeof(int);
		mem_realloc( (void *)&segj.jco, mreq ,4);
		mreq = (size_t)segj.maxcon;
		mreq *= sizeof(double);
		mem_realloc( (void *) &segj.ax, mreq ,5);
		mem_realloc( (void *) &segj.bx, mreq ,6);
		mem_realloc( (void *) &segj.cx, mreq ,7);
	  }
  ...
  do
  {
	...
	if( jcox != j)
	{
	  ...
	  if( segj.jsno >= segj.maxcon )
	  {
		...
		size_t mreq = (size_t)segj.maxcon;
		mreq *= sizeof(int);
		mem_realloc( (void *)&segj.jco, mreq , 8);
		mreq = (size_t)segj.maxcon;
		mreq *= sizeof(double);
		mem_realloc( (void *) &segj.ax, mreq , 9);
		mem_realloc( (void *) &segj.bx, mreq , 10);
		mem_realloc( (void *) &segj.cx, mreq , 11);
	  }
  ...
  if( segj.jsno >= segj.maxcon )
  {
	...
	size_t mreq = (size_t)segj.maxcon;
	mreq *= sizeof(int);
	mem_realloc( (void *)&segj.jco, mreq , 12);
	mreq = (size_t)segj.maxcon;
	mreq *= sizeof(double);
	mem_realloc( (void *) &segj.ax, mreq , 13);
	mem_realloc( (void *) &segj.bx, mreq , 14);
	mem_realloc( (void *) &segj.cx, mreq , 15);
  }
